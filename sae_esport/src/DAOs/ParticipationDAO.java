...
public class ParticipationDAO {
	...
	public ParticipationDAO(Connection c) {
		this.dbConnection = c;
	}
	...
	public List<Participer> getAll() throws Exception {
		...
		PreparedStatement st = this.dbConnection.prepareStatement(reqSelectParticipation);
	...
	public boolean add(Participer value) throws Exception {
		...
		PreparedStatement st = this.dbConnection.prepareStatement("SELECT NEXT VALUE FOR idTournoi FROM participer");
		ResultSet rs = st.executeQuery();
		int id = 0;
		if (rs.next()) {
			id = rs.getInt(1);
		...
		st = this.dbConnection.prepareStatement("INSERT INTO participer VALUES (?,?,?)");
		st.setInt(1, id); 
	...
	public boolean update(Participer value) throws Exception {
		...
		PreparedStatement st = this.dbConnection.prepareStatement("UPDATE participer SET idEquipe=?, resultat=? WHERE idTournoi=?");
